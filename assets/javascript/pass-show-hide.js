...
toggleIcon.onclick = () =>{ // Свойство onclick возвращает обработчик события click на текущем элементе.
  if(pswrdField.type === "password"){
    ...
    toggleIcon.classList.add("active"); // add() Этот метод используется для добавления класса к элементу.
  }else{
    pswrdField.type = "password";
    toggleIcon.classList.remove("active"); // remove() Метод используется для удаления класса у элемента.
  }
}
