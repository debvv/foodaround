...
searchIcon.onclick = ()=>{
  ...
  // Метод toggle объекта classList чередует заданный CSS класс элемента: добавляет класс, 
  // если его нет и удаляет, если есть.
  searchIcon.classList.toggle("active");
  searchBar.focus(); 
  // Метод focus устанавливает фокус на элементе (чаще всего на инпуте). 
  // Это значит, что в этом инпуте начнет моргать курсор и вводимый с клавиатуры текст будет попадать 
  // именно в этот инпут.
...
searchBar.onkeyup = ()=>{ // Событие onkeyup возникает, когда пользователь отпускает нажатую клавишу клавиатуры. 
  ...
  xhr.onload = ()=>{ 
    // Описание Событие onload используется как указатель, что веб-страница полностью загружена, включая содержание,
    //  изображения, стилевые файлы и внешние скрипты.
...
setInterval(() =>{ 
  ...
  // Первым параметром следует передавать функцию-коллбэк, а вторым - время в миллисекундах, указывающее, 
  // через какой промежуток будет повторяться код, заданный первым параметром.
  let xhr = new XMLHttpRequest(); 
